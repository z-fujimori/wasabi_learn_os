...
impl PciXhciDriver {
    ...
    async fn run(xhc: Controller) -> Result<()> {
        ...
        if let Some(port) = connected_port {
            ...
            let device_descriptor = Self::request_device_descriptor(&xhc, slot, &mut ctrl_ep_ring).await?;
    ...
    async fn init_port(xhc: &Rc<Controller>, port: usize) -> Result<u8> {
        ...
        let slot = xhc.send_command(GenericTrbEntry::cmd_enable_slot()).await?.slot_id();
        Ok(slot)
    }
